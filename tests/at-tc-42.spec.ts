...
function getSignupPassword(): string {
  // Signup should not require hardcoded secrets; use env var.
...
class SignupPage {
  ...
  async assertOnSignupPage(): Promise<void> {
    // Some apps render a combined auth page with a "Sign up" tab.
    ...
    // Be tolerant to different signup forms: email/username/name/password.
    const formField = this.emailTextbox
